...
Original file is located at
    https://colab.research.google.com/drive/1PjoSCPbpfqUxx0y506FaKe9Goc-RXcGX
"""
...
# Train and Evaluate Meta-Model using Gradient Boosting
...
meta_model_gb = GradientBoostingClassifier(n_estimators=150, learning_rate=0.05, random_state=42)
meta_model_gb.fit(X_train, y_train)
meta_preds_gb = meta_model_gb.predict(X_test)

# Evaluate the Gradient Boosting meta-model
meta_accuracy_gb = accuracy_score(y_test, meta_preds_gb)
print(f"Gradient Boosting Meta-Model Accuracy: {meta_accuracy_gb}")

# ROC-AUC Score Calculation for Gradient Boosting Meta-Model
meta_preds_proba_gb = meta_model_gb.predict_proba(X_test)
meta_roc_auc_gb = roc_auc_score(pd.get_dummies(y_test).values, meta_preds_proba_gb, multi_class='ovr')
print(f"Gradient Boosting Meta-Model ROC-AUC Score: {meta_roc_auc_gb}")

"""## Training and Validation Loss Curves for DistilBERT"""
...
"""## ROC Curve for Gradient Boosting Meta-Model"""
...
for i in range(n_classes):
    fpr[i], tpr[i], _ = roc_curve(y_test_bin[:, i], meta_preds_proba_gb[:, i])
...
plt.title('Receiver Operating Characteristic (ROC) Curve for Gradient Boosting Meta-Model')
plt.legend(loc="lower right")
plt.show()

# ROC-AUC Score Calculation for Gradient Boosting Meta-Model (overall)
meta_roc_auc_gb = roc_auc_score(y_test_bin, meta_preds_proba_gb, average='macro')
print(f"Gradient Boosting Meta-Model ROC-AUC Score: {meta_roc_auc_gb}")
